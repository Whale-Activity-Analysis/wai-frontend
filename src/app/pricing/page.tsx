        ...
        <div className="container mx-auto px-4 py-8">
            ...
            <div className="text-center max-w-2xl mx-auto">
                <h1 className="text-3xl md:text-5xl font-bold text-neutral-900 dark:text-white mb-4 tracking-tight">
                    {String(t('pricing_title_start', 'Transparente Preise für'))} <span className="text-orange-500">{String(t('pricing_title_highlight', 'jedes Level'))}</span>
                </h1>
                <p className="text-neutral-500 dark:text-neutral-400 text-lg">
                    {String(t('pricing_subtitle', 'Starte kostenlos mit Basis-Metriken oder hol dir den vollen Informationsvorsprung der Wale.'))}
                </p>
            </div>
        </div>
      </div>
      ...
      <main className="container mx-auto px-4 py-16">
        ...
        <div className="grid md:grid-cols-2 gap-8 max-w-5xl mx-auto">
            ...
            <Card className="flex flex-col border-neutral-200 dark:border-neutral-800 shadow-sm hover:shadow-md transition-all bg-white dark:bg-neutral-900">
                <CardHeader>
                    <CardTitle className="text-2xl font-bold text-neutral-900 dark:text-white">{String(t('plan_starter_title', 'Starter'))}</CardTitle>
                    <CardDescription className="text-neutral-500 dark:text-neutral-400">
                        {String(t('plan_starter_desc', 'Perfekt, um den Markt zu beobachten.'))}
                    </CardDescription>
                </CardHeader>
                <CardContent className="flex-1">
                    <div className="mb-6">
                        <span className="text-4xl font-bold text-neutral-900 dark:text-white">0€</span>
                        <span className="text-neutral-500 dark:text-neutral-400 ml-2">/ {String(t('per_month', 'Monat'))}</span>
                    </div>
                    ...
                    <ul className="space-y-3 text-sm text-neutral-600 dark:text-neutral-300">
                        <li className="flex items-center">
                            ...
                            {String(t('feat_wai_live', 'Activity Index (WAI) Live'))}
                        </li>
                        <li className="flex items-center">
                            <Check className="h-4 w-4 text-green-500 mr-2 shrink-0" />
                            {String(t('feat_btc_overlay', 'Bitcoin Preis Overlay'))}
                        </li>
                        <li className="flex items-center">
                            <Check className="h-4 w-4 text-green-500 mr-2 shrink-0" />
                            {String(t('feat_netflow', 'Exchange Netflow (24h)'))}
                        </li>
                        <li className="flex items-center">
                            <Check className="h-4 w-4 text-green-500 mr-2 shrink-0" />
                            {String(t('feat_volume_basic', 'Basis On-Chain Volumen'))}
                        </li>
                        {/* Nicht enthalten */}
                        <li className="flex items-center text-neutral-400 dark:text-neutral-600">
                            <X className="h-4 w-4 mr-2 shrink-0" />
                            {String(t('feat_wii_score', 'Whale Intent (WII) Score'))}
                        </li>
                        <li className="flex items-center text-neutral-400 dark:text-neutral-600">
                            <X className="h-4 w-4 mr-2 shrink-0" />
                            {String(t('feat_smart_signals', 'Smart Signals (Buy/Sell)'))}
                        </li>
                        <li className="flex items-center text-neutral-400 dark:text-neutral-600">
                            <X className="h-4 w-4 mr-2 shrink-0" />
                            {String(t('feat_adv_charts', 'Erweiterte Chart-Tools'))}
                        </li>
                    </ul>
                </CardContent>
                <CardFooter>
                    <Link href="/register" className="w-full">
                        <Button variant="outline" className="w-full border-neutral-200 dark:border-neutral-700 hover:bg-neutral-50 dark:hover:bg-neutral-800 text-neutral-900 dark:text-white">
                            {String(t('start_free_button', 'Kostenlos starten'))}
                        </Button>
                    </Link>
                </CardFooter>
            </Card>
            ...
            <Card className="flex flex-col border-orange-500 dark:border-orange-500 shadow-xl relative bg-white dark:bg-neutral-900 scale-100 md:scale-105 z-10">
                ...
                <div className="absolute -top-4 left-1/2 -translate-x-1/2 bg-orange-500 text-white text-xs font-bold px-3 py-1 rounded-full uppercase tracking-wider shadow-lg">
                    {String(t('most_popular', 'Am Beliebtesten'))}
                </div>

                <CardHeader>
                    <CardTitle className="text-2xl font-bold text-neutral-900 dark:text-white">{String(t('plan_pro_title', 'Pro Trader'))}</CardTitle>
                    <CardDescription className="text-neutral-500 dark:text-neutral-400">
                        {String(t('plan_pro_desc', 'Für alle, die den Markt schlagen wollen.'))}
                    </CardDescription>
                </CardHeader>
                <CardContent className="flex-1">
                    <div className="mb-6">
                    ...
                    <ul className="space-y-3 text-sm text-neutral-600 dark:text-neutral-300">
                        <li className="flex items-center">
                            ...
                            <strong>{String(t('feat_all_starter', 'Alles aus Starter'))}</strong>
                        </li>
                        <li className="flex items-center">
                            <Check className="h-4 w-4 text-orange-500 mr-2 shrink-0" />
                            <span>{String(t('intent_wii', 'Whale Intent Index (WII)'))}</span>
                        </li>
                        <li className="flex items-center">
                            <Check className="h-4 w-4 text-orange-500 mr-2 shrink-0" />
                            <span>{String(t('feat_smart_signals_full', 'Smart Signals (Akkumulation/Distribution)'))}</span>
                        </li>
                        <li className="flex items-center">
                            <Check className="h-4 w-4 text-orange-500 mr-2 shrink-0" />
                            {String(t('feat_adv_flowcharts', 'Erweiterte Flow-Charts'))}
                        </li>
                        <li className="flex items-center">
                            <Check className="h-4 w-4 text-orange-500 mr-2 shrink-0" />
                            {String(t('feat_full_history', 'Volle Historie (kein Limit)'))}
                        </li>
                        <li className="flex items-center">
                            <Check className="h-4 w-4 text-orange-500 mr-2 shrink-0" />
                            {String(t('feat_priority_support', 'Priorisierter Support'))}
                        </li>
                    </ul>
                </CardContent>
                <CardFooter>
                    <Link href="/register?plan=pro" className="w-full">
                        <Button className="w-full bg-orange-500 hover:bg-orange-600 text-white font-bold py-6">
                            {String(t('upgrade_now_button', 'Jetzt Upgraden'))}
                        </Button>
                    </Link>
                </CardFooter>
            </Card>
        ...
        <div className="max-w-3xl mx-auto mt-24">
            <h2 className="text-2xl font-bold text-center text-neutral-900 dark:text-white mb-8">
                {String(t('faq_title', 'Häufig gestellte Fragen'))}
            </h2>
            <div className="grid gap-6">
                <div className="bg-neutral-50 dark:bg-neutral-900/50 p-6 rounded-xl border border-neutral-100 dark:border-neutral-800">
                    <h3 className="font-semibold text-neutral-900 dark:text-white flex items-center gap-2 mb-2">
                        ...
                        {String(t('faq_cancel_q', 'Kann ich jederzeit kündigen?'))}
                    </h3>
                    <p className="text-sm text-neutral-600 dark:text-neutral-400">
                        {String(t('faq_cancel_a', 'Ja, das Abonnement ist monatlich kündbar. Es gibt keine versteckten Kosten oder lange Vertragslaufzeiten.'))}
                    </p>
                </div>
                <div className="bg-neutral-50 dark:bg-neutral-900/50 p-6 rounded-xl border border-neutral-100 dark:border-neutral-800">
                    <h3 className="font-semibold text-neutral-900 dark:text-white flex items-center gap-2 mb-2">
                        <HelpCircle className="h-4 w-4 text-orange-500" />
                        {String(t('faq_payment_q', 'Welche Zahlungsmethoden werden akzeptiert?'))}
                    </h3>
                    <p className="text-sm text-neutral-600 dark:text-neutral-400">
                        {String(t('faq_payment_a', 'Wir akzeptieren alle gängigen Kreditkarten (Visa, Mastercard, Amex) sowie PayPal. Krypto-Zahlungen folgen bald.'))}
                    </p>
                </div>
            </div>
        </div>
